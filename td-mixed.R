...
Ext <- dplyr::filter(X, Phase == "Extinction", ID != "RS34") %>%
...
relief_onset <- dplyr::filter(Ext, TimingCS == "Onset")$Relief
relief_offset <- dplyr::filter(Ext, TimingCS == "Offset")$Relief
relief <- cbind(relief_onset, relief_offset)
relief <- aperm(array(relief, dim = c(T, N, O)), c(2, 1, 3))
...
model <- stan_model('td-mixed.stan')
d <- list(
    N = N, T = T, O = O, Ncs = Ncs, relief = relief, cs = cs,
    # anxious = anxious,
...
y_pred_mu <- apply(y_pred, c(2,3, 4), mean)
